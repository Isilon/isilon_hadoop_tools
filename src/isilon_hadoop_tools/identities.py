    ...
    def add_user_to_group(self, user_name, group_name):
        ...
        try:
            ...
            if exc.user_already_in_group_error(uid, group_name):
                LOGGER.warning(
                    exc.user_already_in_group_error_format.format(uid, group_name)
                )
            else:
                raise
        if self.script_path:
        ...
        while True:
            try:
                ...
                if exc.group_already_exists_error(group_name):
                    LOGGER.warning(
                        exc.group_already_exists_error_format.format(group_name)
                    )
                    gid = self.onefs.gid_of_group(
                        group_name=group_name, zone=self.onefs_zone
                    )
                    break
                raise
        if self.script_path:
    ...
    def create_proxy_user(self, proxy_user_name, members):
        ...
        try:
            ...
            if exc.proxy_user_already_exists_error(proxy_user_name=proxy_user_name):
                LOGGER.warning(
                    exc.proxy_user_already_exists_error_format.format(proxy_user_name)
                )
                return
            raise
        ...
        while True:
            try:
                ...
                if exc.user_already_exists_error(user_name):
                    LOGGER.warning(
                        exc.user_already_exists_error_format.format(user_name)
                    )
                    uid = self.onefs.uid_of_user(
                        user_name=user_name, zone=self.onefs_zone
                    )
                    break
                raise
        if self.script_path:
...
def with_suffix_applied(
    ...
    return {
        "groups": {
            applicator(group_name, suffix) for group_name in identities["groups"]
        },
        "users": {
