...
class Creator:
    ...
    def create_directories(
        self, directories, setup=None, mkdir=None, chmod=None, chown=None
    ):
        ...
        hdfs_root = self.onefs.hdfs_settings(zone=self.onefs_zone)[
            "root_directory"
        ].rstrip(sep)
        ...
        for directory in directories:
            ...
            path = posixpath.join(zone_root, zone_path)
            ...
            try:
                ...
                if exc.dir_path_already_exists_error():
                    ...
                    raise
            LOGGER.info("chmod '%o' '%s'", directory.mode, path)
            (chmod or self.onefs.chmod)(zone_path, directory.mode, zone=self.onefs_zone)
            LOGGER.info("chown '%s:%s' '%s'", directory.owner, directory.group, path)
    ...
    def log_directories(self, directories):
        ...
        self.create_directories(
            directories, setup=_pass, mkdir=_pass, chmod=_pass, chown=_pass
        )
