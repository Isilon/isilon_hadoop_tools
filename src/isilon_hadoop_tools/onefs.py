...
class APIError(_BaseAPIError):
    ...
    user_already_in_group_error_format = (
        "Failed to add member UID:{0} to group GROUP:{1}:"
    ...
    def gid_already_exists_error(self, gid):
        ...
        return any(
            self.filtered_errors(
                ...
                == self.gid_already_exists_error_format.format(gid),
            )
        )
    ...
    def group_not_found_error(self, group_name):
        ...
        return any(
            self.filtered_errors(
                lambda error: error["message"]
                == self.group_not_found_error_format.format(
                    group_name,
                ),
            )
        )
    ...
    def group_unresolvable_error(self, group_name):
        ...
        return any(
            self.filtered_errors(
                lambda error: error["message"]
                == self.group_unresolvable_error_format.format(
                    group_name,
                ),
            )
        )
    ...
    def proxy_user_already_exists_error(self, proxy_user_name):
        ...
        return any(
            self.filtered_errors(
                lambda error: error["message"]
                == self.proxy_user_already_exists_error_format.format(
                    proxy_user_name,
                ),
            )
        )
    ...
    def try_again_error(self):
        ...
        return any(
            self.filtered_errors(
                lambda error: error["message"] == self.try_again_error_format,
            )
        )
    ...
    def uid_already_exists_error(self, uid):
        ...
        return any(
            self.filtered_errors(
                lambda error: error["message"]
                == self.uid_already_exists_error_format.format(uid),
            )
        )
    ...
    def user_already_exists_error(self, user_name):
        ...
        return any(
            self.filtered_errors(
                lambda error: error["message"]
                == self.user_already_exists_error_format.format(
                    user_name,
                ),
            )
        )
    ...
    def user_already_in_group_error(self, uid, group_name):
        ...
        return any(
            self.filtered_errors(
                lambda error: error["message"]
                == self.user_already_in_group_error_format.format(
                    uid,
                    group_name,
                ),
            )
    ...
    def user_not_found_error(self, user_name):
        ...
        return any(
            self.filtered_errors(
                lambda error: error["message"]
                == self.user_not_found_error_format.format(
                    user_name,
                ),
            )
        )
    ...
    def user_unresolvable_error(self, user_name):
        ...
        return any(
            self.filtered_errors(
                lambda error: error["message"]
                == self.user_unresolvable_error_format.format(
                    user_name,
                ),
            )
        )
...
class BaseClient:  # pylint: disable=too-many-public-methods,too-many-instance-attributes
    ...
    def _groups(self, zone=None):
        return (
            self._sdk.AuthApi(self._api_client)
            .list_auth_groups(
    ...
    def _keytab_entries(self, provider):
        providers_krb5 = self._sdk.AuthApi(self._api_client).get_providers_krb5_by_id(
            provider
        )
    ...
    def _pools(self, *args, **kwargs):
        return (
            self._sdk.NetworkApi(self._api_client)
            .get_network_pools(*args, **kwargs)
            .pools
        )
    ...
    def _refresh_sdk(self):
        try:
            self._revision = (  # pylint: disable=attribute-defined-outside-init
                self.revision()
            )
    ...
    def add_user_to_group(self, user_name, group_name, zone=None):
        ...
        group_member_cls = (
            ...
            else self._sdk.AuthAccessAccessItemFileGroup
        )
        try:
            ...
            assert all(
                [
                    ...
                    in [
                        member.name
                        for member in self._sdk.AuthGroupsApi(self._api_client)
                        .list_group_members(
                            group_name,
                            zone=zone or self.default_zone,
                        )
        ...
        try:
            ...
            assert all(
                [
                    ...
                    in [
                        krb5.realm
                        for krb5 in auth_api.list_settings_krb5_realms().realm
                    ],
                ]
            )
    ...
    @accesses_onefs
    def create_user(
        self, name, primary_group_name, uid=None, zone=None, enabled=None, password=None
    ):
        """Create a user."""
        group_member_cls = (
            self._sdk.GroupMember
    ...
    def feature_is_supported(self, feature):
        ...
        entries_for_gen = [
            entry.bits
            for entry in committed_features.gen_bits
    ...
    def gid_of_group(self, group_name, zone=None):
        ...
        assert (
            len(auth_groups.groups) == 1
        ), "Do you have duplicate groups (e.g. local and LDAP)?"
    ...
    def has_license(self, name):
        ...
        return any(
            _license_is_active(license) for license in self._license(name).licenses
        )
    ...
    def hdfs_inotify_settings(self, zone=None):
        ...
        try:
            hdfs_inotify_settings = (
                self._sdk.ProtocolsApi(
                    self._api_client,
                )
    ...
    def hdfs_settings(self, zone=None):
        ...
        hdfs_settings = (
            self._sdk.ProtocolsApi(self._api_client)
            .get_hdfs_settings(
                zone=zone or self.default_zone,
            )
            .settings
    ...
    def list_spns(self, provider):
        ...
        return [
            keytab_entry.spn for keytab_entry in self._keytab_entries(provider=provider)
        ]
    ...
    def node_addresses(self, zone=None):
        ...
        return {
            ...
            for pool in self._sdk.NetworkApi(self._api_client)
            .get_network_pools(
    ...
    def primary_group_of_user(self, user_name, zone=None):
        ...
        assert (
            len(auth_users.users) == 1
        ), "Do you have duplicate users (e.g. local and LDAP)?"
